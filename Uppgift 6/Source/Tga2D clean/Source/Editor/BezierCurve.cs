...
namespace Curves
{
    class BezierCurve
    {
        ...
        public void Generate(double[] pointsIn, int cpts, double[] pointsOut)
        {
            int npts = (pointsIn.Length) / 2;
            int icount, jcount;
            double step, t;
            ...
            { 
                ...
                jcount = 0;
                pointsOut[icount] = 0.0;
                pointsOut[icount + 1] = 0.0;
                for (int i = 0; i != npts; i++)
                {
                    double basis = Bernstein(npts - 1, i, t);
                    pointsOut[icount] += basis * pointsIn[jcount];
                    pointsOut[icount + 1] += basis * pointsIn[jcount + 1];
                    jcount = jcount +2;
                }
