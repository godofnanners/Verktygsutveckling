...
// This is a struct that only lives on the engine/loading side. Gameplay should never ever touch a TGA2D::CSprite
...
//This lives on the gameplay side, the only thing connecting this to a CSprite is the ID
...
void RunEngineThread()
{
	...
	while (true)
	{
		...
		for (SSpriteResourceGame& gameSprite : torenderSprites)
		{
			...
			{
				myEngineSprites[gameSprite.myID].mySprite->SetPosition(gameSprite.myPosition);
				myEngineSprites[gameSprite.myID].mySprite->Render();
			}
...
void RunLoadingThread()
{
	...
	while (!myShouldExit)
	{
		...
		if (myToLoadResources.size() > 0)
		{
			...
			newEngineResource.mySprite = new Tga2D::CSprite(myToLoadResources[0].c_str());
			newEngineResource.mySprite->SetPivot({ 0.5f, 0.5f });
